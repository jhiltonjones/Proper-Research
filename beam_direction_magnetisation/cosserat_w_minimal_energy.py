...
class CosseratForwardModel:
    ...
    def solve(self, *, L, r_src, m_src, try_energy_selection=True):
        L = float(L)
        ...
        # If you don't want tie-breaking, keep your current warm-start logic:
        ...
        # --- Energy tie-breaker branch exploration ---
        guesses = []
        ...
        for (s_mesh, Y_guess, tag) in guesses:
            ...
            candidates.append((W, sol, tag, parts))
...
def compute_total_energy(sol, *, L, r_src, m_src, Kinv_fun, m_local_fun, m_moment,
    ...
    W_total = W_s + W_m + W_v
    parts = dict(W_s=W_s, W_m=W_m, W_v=W_v)
