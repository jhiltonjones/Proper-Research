...
def magnetic_wrench_density_cosserat_profile(p, q, s, m_ext, r_src, m_local_fun, r_min=1e-6):
    """
    p: (3,N), q: (4,N), s: (N,)
    ...
    m_loc = m_local_fun(s)                 # (3,N) body frame
...
ell_m = 0.01
s_m = 0.032
mu_tip = mu_line   # or your chosen magnetised moment/length
m_tip_local = np.array([mu_tip, 0, 0])
def m_local_profile(s):
    mask = ((s >= s_m) & (s <= s_m + ell_m)).astype(float)
    return m_tip_local[:, None] * mask[None, :]
...
s_k = 0.032  # start of "composite/PDMS" segment [m]

# Define stiffness values for two segments
EI1, GJ1 = EI, GJ          # base segment (e.g., nitinol)
EI2, GJ2 = 0.3*EI, 0.3*GJ  # tip composite example (replace with real values)

def Kbt_inv_profile(s):
    """
    s: (N,) array
    returns Kinv: (3,3,N) such that u(:,i) = Kinv(:,:,i) @ m_body(:,i)
    """
    mask = (s >= s_k)
...
def make_cosserat_kirchhoff_ode(m_src, r_src, Kinv_fun, m_local_fun, u_star=None):
    ...
    def ode(s, Y):
        ...
        # inextensible kinematics
        p_s = np.einsum('nij,j->ni', R, e1).T

        # constitutive with varying stiffness
        ...
        f_ext, tau_ext, _B = magnetic_wrench_density_cosserat_profile(
            p, qn, s, m_src, r_src, m_local_fun, r_min=1e-6
        )
...
scales = [1.0]
sol = None
def solve_for_pose(r_src, m_src, Y_guess, s_mesh, sol=None):
...
for ang in angles_deg:
    ...
    f_ext, tau_ext, B = magnetic_wrench_density_cosserat_profile(
        p_f, q_f, s_out, m_src_f, r_src_f, m_local_profile, r_min=1e-6
    )
    ...
    f_ext_o, tau_ext_o, B_o = magnetic_wrench_density_cosserat_profile(
        p_o, q_o, s_out, m_src_o, r_src_o, m_local_profile, r_min=1e-6
    )
...
m_loc = m_local_profile(s_out)          # (3,N)
