...
def numerical_B_tip_xyz_wrt_u(p8, forward_fn, dt, eps_u):
    """
    Returns B (3x7) s.t. x_next ≈ x_now + B u
    by finite-differencing u and rolling pose one step with quaternion integration.
    """
    p8 = np.asarray(p8, float).ravel()
    eps_u = np.asarray(eps_u, float).ravel()
    assert eps_u.size == 7

    x0 = np.asarray(forward_fn(p8), float).reshape(3,)
    B = np.zeros((3, 7), float)
    ...
    for i in range(7):
        ...
        x_plus  = np.asarray(forward_fn(p_plus), float).reshape(3,)
        x_minus = np.asarray(forward_fn(p_minus), float).reshape(3,)

        # derivative of x (absolute) w.r.t u around u=0
        B[:, i] = (x_plus - x_minus) / (2.0 * eps_u[i])
...
if __name__ == "__main__":
    ...
    J_fn = lambda p8: numerical_B_tip_xyz_wrt_u(p8, forward_tip_det, dt=0.05, eps_u=eps_u)    # p0 = np.array([ 0.63360145, -0.56541852,  0.20705173, -3.116988654350607, 0.19059356279735162, 0.028215660130034903,L0])
    ...
    for k in range(max_steps):
        ...
        if np.linalg.norm(xref_seq[0] - x_post) < 3e-3: 
