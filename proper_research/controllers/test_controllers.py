...
try:
    from proper_research.simulation.simulations import simulate_time_parameterized_beam_output_mpc as beam_module
    from proper_research.simulation.simulations import simulate_time_parameterized_configuration_mpc as base
except ModuleNotFoundError as error:  # a clear message beats a bare traceback
    raise SystemExit(
        f"Could not import {error.name}.\n\n"
        "This test builds the four controllers against the REAL MPC modules "
        "rather than mocks of them — that is most of what it proves — so it "
        "must run somewhere those two modules are importable. Put this file "
        "beside simulate_time_parameterized_configuration_mpc.py and "
        "simulate_time_parameterized_beam_output_mpc.py, or run it with the "
        "project package on PYTHONPATH.\n\n"
        "The contact-study tests (experiments/) have no such requirement and "
        "run anywhere."
    ) from error
from proper_research.controllers.inverse_jacobian_controller import build_inverse_jacobian_controller
from proper_research.controllers.mpc_variants import build_all_mpc_variants, precompute_schedule
...
def main() -> None:
    ...
    controllers = build_all_mpc_variants(
        ...
        allow_undeclared_jacobian=True,  # mock callables, not real providers
    )
