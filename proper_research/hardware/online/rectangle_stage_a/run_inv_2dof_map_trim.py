#!/usr/bin/env python3
"""INV 2DOF trim + learned path-indexed feedforward map, through execution
layer C. See `proper_research/controllers/inverse_jacobian_2dof_map_trim.py`
for the architecture, and this package's README for the full chain that
produced the map (advancer-excitation ring-down test -> ruled out -> open-
loop delay/lag analysis -> repeatability ratio R_d=0.922 -> leave-one-out
map validation -> lambda=1000 curvature-regularized smoothing sweep).

Controller is otherwise IDENTICAL to inv_2dof_trim at its frozen kp=0.6,
kn=0 development point -- this script exists to isolate exactly one
change (the map term) for the live A/B against run_inv_2dof_trim.py.

Needs the SAME genuine LTV schedule run_mpc_ltv.py builds/caches (the map
term's Jacobian must be the schedule that validated it offline, not a
live-state jacobian_provider re-evaluation -- see the controller module's
__init__ docstring) plus the smoothed dmap .npz.

Usage
-----
    python -m proper_research.hardware.online.rectangle_stage_a.run_inv_2dof_map_trim \\
        --plan-dir plans/rectangle_10x15mm_bimaterial/time_parameterized_configuration_path \\
        --out-dir close_loop_logs/my_run \\
        --dmap-path close_loop_logs/.../dmap_final_lambda1000.npz \\
...
def build_or_load_schedule(plan_dir: str, cache_path: str | None) -> np.ndarray:
    if cache_path and os.path.exists(cache_path):
        print(f"[inv_2dof_map_trim] loading cached genuine LTV schedule from {cache_path}")
        return np.load(cache_path)
    print("[inv_2dof_map_trim] building genuine from_model_bundle Jacobian schedule "
    ...
    if cache_path:
        ...
        print(f"[inv_2dof_map_trim] schedule cached -> {cache_path}")
    return schedule
...
def main() -> None:
    ...
    p.add_argument("--run-name", default=None, help="default: inv_2dof_map_trim_kp<kp>_kn<kn>")
    p.add_argument("--dmap-path", required=True)
    ...
    print(f"[inv_2dof_map_trim] schedule: {schedule.shape}")
    ...
    cfg = pf.CONFIG
    cfg.controller_kind = "inv_2dof_map_trim"
    cfg.plan_dir = args.plan_dir
    cfg.reference_source = "plan_dir"
    cfg.dmap_path = args.dmap_path
    ...
    cfg.run_name = args.run_name or f"inv_2dof_map_trim_kp{args.kp:g}_kn{args.kn:g}"

    print(
        f"[inv_2dof_map_trim] plan={args.plan_dir}\n"
        f"[inv_2dof_map_trim] q0={q0.tolist()} L0={l0*1000:.2f}mm\n"
        f"[inv_2dof_map_trim] dmap={args.dmap_path}\n"
        f"[inv_2dof_map_trim] kp={args.kp} kn={args.kn} damping={args.damping} q_trim_max={args.q_trim_max}\n"
        f"[inv_2dof_map_trim] servo_stream_hz={cfg.servo_stream_hz} "
