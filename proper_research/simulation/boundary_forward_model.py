...
from beam_direction_magnetisation.cosserat_w_minimal_energy import (
    dipole_from_pose, LumenQuery
)

from beam_direction_magnetisation.ana_energy import (
    solve_quasistatic_insertion,
    make_energy_grad_fun_for_pose,
    energy_min_tip_jacobian_implicit,
)
...
from beam_direction_magnetisation.ana_energy import make_energy_grad_fun_for_pose, energy_min_tip_jacobian_implicit
...
#         return J_tip
#     def analytic_tip_jacobian_p7(self, p7):
#         """
#         Returns J_tip wrt [r_src_x, r_src_y, r_src_z, dphi_x, dphi_y, dphi_z, L]
#         shape = (3, 7)

#         Assumes self(p7) has just been called so last_info contains u_flat_opt.
#         """
#         p7 = np.asarray(p7, float).reshape(7,)
...
#             raise RuntimeError("Call forward model once before requesting analytic Jacobian.")

#         if "u_flat_opt" not in self.last_info:
#             raise RuntimeError("last_info has no u_flat_opt. Cannot build implicit Jacobian.")

#         u_ref = self.last_info["u_flat_opt"]
...
#         self.last_jacobian_info = sens_info
#         return J_tip_theta
...
class EnergyMinForwardWithAnalyticJac(EnergyMinForwardWithLumen):
    ...
    def jacobian_tip_pose7_from_cache(
        self,
        p7,
        *,
        eps_theta=1e-3,
        ...
        Does NOT call self(p7).
        """
        p7 = np.asarray(p7, float).reshape(7,)

        if self.last_info is None or "u_flat_opt" not in self.last_info:
            raise RuntimeError(
    ...
    def jacobian_tip_pose7(
        ...
        *,
        ...
        By default this does NOT solve.
        Set solve_if_needed=True only for standalone debugging.
        """
        if self.last_info is None:
            if not solve_if_needed:
                raise RuntimeError(
                    "No cached forward solve. Call self(p7) first or pass solve_if_needed=True."
                )
            self(p7)

        return self.jacobian_tip_pose7_from_cache(
            p7,
            eps_theta=eps_theta,
            eps_hess=eps_hess,
        )