...
def make_plots(summary):
    ...
    #
    # The original script used horizon-weighted budget on the x-axis. That is
    # still useful as a horizon-aware proxy, but for your current comparison it
    # is clearer to also plot:
    #   1. MPC updates vs accuracy
    #   2. SQP iterations / solve-count proxy vs accuracy
    #
    # Interpretation:
    # - n_mpc_updates answers: "How many controller calls were needed?"
    # - sum_sqp_iters_done answers: "How many SQP iterations or LTI solve-count
    #   proxy units were used?"
    # - horizon_weighted_budget_sum_Nsqp_x_rollout answers: "How much work if
    #   larger rollout horizons are treated as more expensive?"
    def plot_cost_accuracy_scatter(x_metric, x_label, filename):
        if x_metric not in summary.columns or "p95_one_step_xy_mm" not in summary.columns:
            return
        ...
        for group, g in summary.groupby("group"):
            ...
            # Annotate each point with the bend angle.
            for _, r in g.iterrows():
                if np.isfinite(r["bend_abs_deg"]) and np.isfinite(r[x_metric]) and np.isfinite(r["p95_one_step_xy_mm"]):
                    ax.annotate(
                        int(r["bend_abs_deg"]),
                        (r[x_metric], r["p95_one_step_xy_mm"]),
                        fontsize=8,
                    )

        ax.set_xlabel(x_label)
        ax.set_ylabel("P95 one-step XY error [mm]")
        ax.set_title(f"Cost–accuracy trade-off: {x_label}")
    ...
    plot_cost_accuracy_scatter(
        "n_mpc_updates",
        "MPC updates",
        "cost_accuracy_tradeoff_mpc_updates.png",
    )

    plot_cost_accuracy_scatter(
        "sum_sqp_iters_done",
        "Total SQP iterations / LTI solve-count proxy",
        "cost_accuracy_tradeoff_sqp_iterations.png",
    )

    plot_cost_accuracy_scatter(
        "horizon_weighted_budget_sum_Nsqp_x_rollout",
        "Horizon-weighted budget Σ(Nsqp×rollout)",
        "cost_accuracy_tradeoff_horizon_weighted.png",
    )
...
def main():
    ...
    with open(OUT_DIR / "README.txt", "w") as f:
        ...
        f.write("The script now outputs three cost-accuracy scatter plots:\n")
