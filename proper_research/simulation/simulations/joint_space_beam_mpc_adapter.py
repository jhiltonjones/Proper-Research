...
class JointSpaceBeamMPCAdapter:
    ...
    def reset(self) -> None:
        """Forward an optional reset hook used by the simulation runner."""
        hook = getattr(self.beam_output_fn, "reset", None)
        if callable(hook):
            hook()
...
class _ForwardOutputProxy:
    ...
    @property
    def last_p_centerline(self) -> Array | None:
        """Expose the latest beam centreline to the existing plot runner.

        ``beam_output_fn`` is normally a small transform-to-pose wrapper around
        ``ControllerForwardAdapter``.  The wrapper stores that object as
        ``forward_adapter`` so this proxy can preserve the old runner API.
        """
        sources = (
            self.adapter.beam_output_fn,
            getattr(self.adapter.beam_output_fn, "forward_adapter", None),
            self.adapter.model,
        )
        for source in sources:
            if source is None:
                continue
            value = getattr(source, "last_p_centerline", None)
            if value is not None:
                return np.asarray(value, dtype=float).copy()
        return None

    def get_last_diag(self) -> dict[str, Any]:
        return self.adapter.get_last_diag()
